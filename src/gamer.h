...
class TGamer {
   public:
      ...
      TGamer(int _nGamer);
      TGamer(int _nGamer,TDeskView *_DeskView);
      // networking part
      char *NikName;
      char *NikPasswd;
      char *Message;
      int nNetworkGamer; // номер игрока на сервере
      int nNetworkDesk; // network desk number
      int MasterNumber; // для шланга номер мастера
      int socket;
      virtual int connecttoserver(char *,unsigned short );
